...
function getEventHandler(appModule, controllerBasePath, handlerName, isMiddleware = false) {
    ...
    if (pos < 0) {
        if (isMiddleware)
            appModule.invalidConfig('socketRpc.channels.' + name + '.middlewares', `Invalid middleware reference: ${handlerName}`);
        else
            appModule.invalidConfig('socketRpc.channels.' + name + '.events', `Invalid event handler reference: ${handlerName}`);
    }
    ...
    if (!middlewareHandler) {
        if (isMiddleware)
            appModule.invalidConfig('socketRpc.channels.' + name + '.middlewares', `Middleware function not found: ${handlerName}`);
        else
            appModule.invalidConfig('socketRpc.channels.' + name + '.events', `Event handler function not found: ${handlerName}`);
    }
...
module.exports = {
    ...
    load: function (appModule, config) {
        appModule.on('after:' + Util.Feature.ENGINE, () => {
            ...
            let logger;
            if (config.logger) {
                logger = appModule.getLoggerById(config.logger);
            }

            if (Util._.isEmpty(config.channels)) {
                appModule.invalidConfig('socketRpc.channels', 'Missing channels config.');
            }

            let controllerPath = appModule.toAbsolutePath(appModule.options.backendPath, config.controllerPath || 'socketRpc');
            ...
            Util._.forOwn(config.channels, (info, name) => {
                ...
                if (info.middlewares) {
                    ...
                    m.forEach(middlewareName => {
                        ioChannel.use(getEventHandler(appModule, controllerPath, middlewareName, true));
                    });
                }
                ...
                if (info.events) {
                    Util._.forOwn(info.events, (handler, event) => {
                        eventHandlers[event] = getEventHandler(appModule, controllerPath, handler);
                    });
                }
            ...
            if (standalone && !appModule.serverModule.options.deaf) {
