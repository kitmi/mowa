        ...
        Util._.forOwn(routes, function (routersConfig, route) {
            ...
            if (Util._.isObject(routersConfig)) {
                Util._.forOwn(routersConfig, function (options, type) {
                    ...
                    promises.push(() => {
                        webModule.consoleVerbose(`A "${type}" router is created at "${mount}" in module [${webModule.name}].`);
                ...
                promises.push(() => {
                    webModule.consoleVerbose(`A "rule" router is created at "${mount}" in module [${webModule.name}].`);
