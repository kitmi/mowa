...
exports.startMowa = function (api) {
    let mowa = new Mowa(api.mowaName, {deaf: true, verbose: api.config['default'].mowaVerbose});
    return mowa.start();
};

exports.loadServerConfig_ = function (api) {
    let configLoader = new Config(new JsonConfigProvider(path.join(api.base, 'etc'), 'server', this.env));
    configLoader.autoInterpolate = false;

    return configLoader.load().then(() => configLoader);
};
