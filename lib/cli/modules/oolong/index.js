...
function startMowaAndRunWithSchemaFiles(api, command, callback) {
    ...
    return MowaHelper.startMowa(api).then(server => {
        ...
        async.eachSeries(moduleNames, (moduleName, cb) => {
            ...
            let appModule = server.childModules[moduleName];
            let oolongDir = appModule.toAbsolutePath('oolong');
            ...
            async.eachSeries(schemaFiles, callback(appModule), (err2) => {
...
exports.create  = function (api) {
    ...
    return MowaHelper.getDbConnectionList(api).then(result => {
        ...
        if (Util._.isEmpty(connectionList)) {
            return Promise.reject('No db connection found. Add a db connection by mowa db add first.');
        }
...
exports.deploy = function (api) {
    ...
    return startMowaAndRunWithSchemaFiles(api, 'deploy', appModule => (f, cb) => {
        oolong.deploy(appModule, { logger: api.logger }, f, reset).then(() => cb(), e => cb(e));
    });
};
...
exports.initTest = function (api) {
    ...
    return startMowaAndRunWithSchemaFiles(api, 'initTest', appModule => (f, cb) => {
        oolong.import(appModule, { logger: api.logger }, f, 'test').then(() => cb(), e => cb(e));
    });
};