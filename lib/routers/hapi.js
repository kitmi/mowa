    ...
    Util._.forOwn(options.rules, (methods, subRoute) => {
        ...
        Util._.forOwn(methods, (middlewares, method) => {
            ...
            if (Util._.isString(middlewares)) {
                middlewares = { action: path.join(appModule.options.backendPath, options.controllers, middlewares) };
            } else {
                if ('action' in middlewares) {
                    middlewares['action'] = path.join(appModule.options.backendPath, options.controllers, middlewares['action']);
                }
            }
