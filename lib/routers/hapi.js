...
module.exports = function loadRuleRouter(webModule, baseRoute, options) {
    ...
    if (!options.rules) {
        webModule.invalidConfig('routes.*.rule', 'Missing rules definition.');
    }
    ...
    Util._.forOwn(options.rules, (methods, subRoute) => {
        ...
        if (pos != -1) {
            if (pos == 0) {
                webModule.invalidConfig('routes.*.rule.rules', 'Unrecognized route rule syntax: ' + subRoute);
            }
        ...
        Util._.forOwn(methods, (middlewares, method) => {
            if (!ALLOWED_METHODS.has(method)) {
                webModule.invalidConfig('routes.*.rule.rules', 'Unsupported http method: ' + method);
            }

            if (Util._.isString(middlewares)) {
                middlewares = { action: path.join(webModule.options.backendPath, options.controllers, middlewares) };
            } else {
                if ('action' in middlewares) {
                    middlewares['action'] = path.join(webModule.options.backendPath, options.controllers, middlewares['action']);
                }
            }

            webModule.addRoute(router, method, subRoute, middlewares);
        });
    });
