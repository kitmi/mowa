...
class OolongField {
    ...
    constructor(name, rawInfo) {
        ...
        if (rawInfo.validators) {
             ...
             * @public
             */
            this.validators = rawInfo.validators.map(v => (typeof v === 'string') ? { name: v } : { name: v.name, arguments: v.arguments });
        }
        ...
        if (rawInfo.modifiers) {
             ...
             * @public
             */
            this.modifiers = rawInfo.modifiers.map(m => (typeof m === 'string') ? { name: v } : { name: m.name, arguments: m.arguments });
        }
