    ...
    _compile(oolFile) {
        ...
        if (ool.namespace) {
            ool.namespace.forEach(ns => {
                ...
                if (fs.existsSync(p+'.ool')) {
                    namespace.push(p);
                    return;
                }

                if (fs.existsSync(p)) {
                    namespace.push(path.join(p, '*'));
                    return;
                }

                throw new Error('Referenced path "' + p + '" not exist!');
            });
        }
        ...
        ool.id = isCoreEntity
            ...
            : './' + path.relative(this.currentAppModule.oolongPath, pathWithoutExt);
