...
function deploy(webModule, context, dbName, dbConnectionKey, dbInfo, reset) {
    let { logger } = context;
    
    let dbScriptDir = webModule.toAbsolutePath(webModule.options.backendPath, 'db', 'mysql', dbName);

    let dbService = webModule.getService('mysql:' + dbConnectionKey);
    if (!dbService) {
...
class ActionContext {
    constructor(webModule) {
        this.webModule = webModule;

        let i18n = webModule.getService('i18n');
        if (i18n) {
...
function importData(webModule, context, dbName, dbInfo, dataFile) {
    let { logger } = context;
    ...
    if (ext == '.json') {
        ...
        let querys = [];
        let ctx = new ActionContext(webModule);

        _.forOwn(data, (records, tableName) => {
            console.log(dbName + '.' + tableName);
            let model = webModule.getModel(dbName + '.' + tableName);
        ...
        let dbService = webModule.getService('mysql:' + dbName);
        if (!dbService) {
