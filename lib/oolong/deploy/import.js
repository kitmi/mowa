...
module.exports = function (webModule, context, schemaFile, mode) {
    ...
    _.forOwn(schemaModule.database, (db, dbName) => {
        let dataDir = webModule.toAbsolutePath(webModule.options.backendPath, 'db', db.type, dbName, mode);
        ...
        _.each(dataList, line => {
            ...
            if (line.length > 0) {
                ...
                promises.push(dbDeployer.importData(webModule, context, dbName, db, dataFile));            
            }
        });        
    });
