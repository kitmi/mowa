...
module.exports = {
    ...
    load: function (appModule, config) {        
        ...
        if (!appModule.serverModule.options.deaf) {
            ...
            appModule.on('after:' + Util.Feature.ROUTING, () => {
                appModule.httpServer.listen(port, function (err) {
                    if (err) throw err;

                    if (config.publicUrl) {
                        appModule.publicUrl = config.publicUrl;
                    } else {
